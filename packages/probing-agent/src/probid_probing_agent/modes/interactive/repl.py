"""Minimal terminal REPL for the probid probing agent harness."""
...
def run_agent_repl(runtime: ProbidAgentRuntime) -> None:
    ...
    while True:
        try:
            user_input = click.prompt("harness", prompt_suffix="> ")
        ...
        if cmd == "/mode":
            ...
            click.echo(
                "mode=interactive "
                f"json_mode={json_mode} why_mode={why_mode} cache_only={runtime.default_cache_only} "
        ...
        if cmd == "/reset":
            runtime.new_session()
            click.echo("Started a new harness session.")
            continue
